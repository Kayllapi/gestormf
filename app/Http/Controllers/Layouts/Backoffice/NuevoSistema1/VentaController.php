...
class VentaController extends Controller
{
    public function index(Request $request,$idtienda)
    {
        ...
        json_venta($idtienda,$request->name_modulo);
        /*if($request->input('puntoconsumo')!=''){
            $where[] = ['s_configpuntoconsumo.nombre','LIKE','%'.$request->input('puntoconsumo').'%'];
        }*/
        return view('layouts/backoffice/tienda/nuevosistema/venta/index',[
            'tienda' => $tienda
        ]);
    }
    ...
    public function show(Request $request, $idtienda, $id)
    {
        ...
        elseif ($id == 'show-comida-seleccionarproducto') {
            $pedido = DB::table('s_comida_ordenpedido')
                    ...
                    ->where('s_comida_ordenpedido.codigo',$request->codigopedido)
                    ->select(
                        's_comida_ordenpedido.*',
                        's_comida_mesa.numero_mesa as nombremesa',
                    )
                  ->first();
            $numeromesa = '';
            $mensaje = '';
            ...
            if($pedido!=''){
                if($pedido->idestado==1){
                    $mensaje = 'El Orden de Pedido esta Pendiente!!';
                    ...
                    $numeromesa = 'Mesa '.str_pad($pedido->nombremesa, 2, "0", STR_PAD_LEFT);
                }elseif($pedido->idestado==3){
                    $mensaje = 'El Orden de Pedido esta Vendido!!';
                }elseif($pedido->idestado==4){
                    $mensaje = 'El Orden de Pedido esta Anulado!!';
                }
                    
            }else{
                $mensaje = 'No existe el pedido, ingrese otro código por favor.';
            }
            return [ 
    ...
    public function edit(Request $request, $idtienda, $id)
    {
        ...
        elseif($request->input('view') == 'detalle') {
           $s_ventadetalles = DB::table('s_ventadetalle')
            ...
            $agencia = DB::table('s_agencia')->where('idtienda',$idtienda)->get();
            $comprobante = DB::table('s_tipocomprobante')->get();
            $tipoentregas = DB::table('s_tipoentrega')
              ->where('s_tipoentrega.id',$venta->s_idtipoentrega)
              ->first();
            $tipopersonas = DB::table('tipopersona')->get();
            $monedas = DB::table('s_moneda')->get();

            $configuracion = configuracion_comercio($idtienda);
            return view('layouts/backoffice/tienda/nuevosistema/venta/detalle',[
                'tienda' => $tienda,
                'agencia' => $agencia,
                'comprobante' => $comprobante,
                'tipopersonas' => $tipopersonas,
      ...
      //   dd($s_ventadetalles);
        ...
        elseif($request->input('view') == 'anular') {
            ...
            $configuracion = configuracion_comercio($idtienda);
            return view('layouts/backoffice/tienda/nuevosistema/venta/anular',[
                'tienda' => $tienda,
                'agencia' => $agencia,
                'comprobante' => $comprobante,
                'tipopersonas' => $tipopersonas,
        ...
        elseif($request->input('view') == 'ticket') {
            return view('layouts/backoffice/tienda/nuevosistema/venta/ticket',[
                'tienda' => $tienda,
                'venta' => $venta
            ]);
        }
        elseif($request->input('view') == 'ticketpdf') {
            ...
            $pdf = PDF::loadView('layouts/backoffice/tienda/nuevosistema/venta/ticketpdf',[
                'tienda' => $tienda,
                'agencia' => $agencia,
                'cliente' => $cliente,
                'vendedor' => $vendedor,
