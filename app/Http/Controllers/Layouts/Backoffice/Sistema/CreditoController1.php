    ...
    public function store(Request $request, $idtienda)
    {
        ...
        if($request->input('view') == 'registrar') {
            ...
            $credito = DB::table('credito')
                //->where('credito.idevaluacion',$credito->idevaluacion)
    ...
    public function edit(Request $request, $idtienda, $id)
    {
      ...
      if($request->input('view') == 'editar') {
        ...
        $credito_evaluacion_cualitativa = DB::table('credito_evaluacion_cualitativa')
          ...
          /*$credito_lista_garantias_cliente = DB::table('credito_garantia')
          ...
          $data_credito_garantias_cliente = [];
          foreach($credito_lista_garantias_cliente as $value){
              if($value->idgarantias!=0){
                $lista_credito_garantia_cliente_propuesta = DB::table('credito')
                  ->join('credito_garantia','credito_garantia.idcredito','credito.id')
                  ->join('credito_prendatario','credito_prendatario.id','credito.idcredito_prendatario')
                  ->where('credito_garantia.idgarantias',$value->idgarantias)
                  ->where('credito_garantia.idcredito',$credito->id)
                  ->where('credito_garantia.idcliente',$credito->idcliente)
                  ->where('credito_garantia.tipo','CLIENTE')
                  ->where('credito.idestadocredito',1)
                  ->select(
                      ...
                      DB::raw('CONCAT("PROPUESTA") as tipoprestamo')
                  );
                ...
                $lista_credito_garantia_cliente_aval = DB::table('credito')
                  ...
                  ->union($lista_credito_garantia_cliente_propuesta)
                  ->union($lista_credito_garantia_cliente_propio)
                  ->get();
              }else{
                $lista_credito_garantia_cliente_propuesta = DB::table('credito')
                  ->join('credito_garantia','credito_garantia.idcredito','credito.id')
                  ->join('credito_prendatario','credito_prendatario.id','credito.idcredito_prendatario')
                  ->where('credito_garantia.idgarantias_noprendarias',$value->idgarantias_noprendarias)
                  ->where('credito_garantia.idcredito',$credito->id)
                  ->where('credito_garantia.idcliente',$credito->idcliente)
                  ->where('credito_garantia.tipo','CLIENTE')
                  ->where('credito.idestadocredito',1)
              ...
              foreach($lista_credito_garantia_cliente_aval as $valuec){
                  ...
                  $cuenta_avalado = '';
                  $cuenta_propuesta = '';
                  $saldo_vigente_propuesta = '';
                  $saldo_vigente_propio = '';
                  $saldo_vigente_avalado = '';
                  if($valuec->tipoprestamo=='PROPUESTA'){
                      $cuenta_propuesta = 'C'.str_pad($valuec->credito_cuenta, 8, "0", STR_PAD_LEFT);
                      $saldo_vigente_propuesta = $cronograma['saldo_capital'];
                  }
                  elseif($valuec->tipoprestamo=='PROPIO'){
                      $cuenta_propio = 'C'.str_pad($valuec->credito_cuenta, 8, "0", STR_PAD_LEFT);
                      $saldo_vigente_propio = $cronograma['saldo_capital'];
                  }
                  elseif($valuec->tipoprestamo=='AVALADO'){
                      $cuenta_avalado = 'C'.str_pad($valuec->credito_cuenta, 8, "0", STR_PAD_LEFT);
                      $saldo_vigente_avalado = $cronograma['saldo_capital'];
                  }
                
                  $data_lista_credito_garantia[] = [
                      'idforma_credito' => $valuec->idforma_credito,
                      'modalidad' => $valuec->modalidadproductocredito,
                      'cuenta_propio' => $cuenta_propio,
                      'cuenta_avalado' => $cuenta_avalado,
                      'cuenta_propuesta' => $cuenta_propuesta,
                      'saldo_vigente_propio' => $saldo_vigente_propio,
                      'saldo_vigente_avalado' => $saldo_vigente_avalado,
                      'saldo_vigente_propuesta' => $saldo_vigente_propuesta,
                  ];
              }
            
              $data_credito_garantias_cliente[] = [
                  'nombretipogarantia' => $value->garantias_noprendarias_tipo_garantia_noprendaria,
                  'descripcion_garantia' => $value->descripcion,
                  'valor_mercado_garantia' => $value->valor_mercado,
                  'valor_comercial_garantia' => $value->valor_comercial,
                  'valor_realizacion_garantia' => $value->valor_realizacion,
                  'tipo_garantia' => $value->tipo,
                  'idgarantias' => $value->idgarantias,
                  'idgarantias_noprendarias' => $value->idgarantias_noprendarias,
                  'creditos' => $data_lista_credito_garantia,
              ];
          }
        
          //-------- GARANTIAS DE AVAL
        
          $credito_lista_garantias_aval = DB::table('credito_garantia')
          ...
          $data_credito_garantias_aval = [];
          foreach($credito_lista_garantias_aval as $value){
              if($value->idgarantias!=0){
                $lista_credito_garantia_aval_propuesta = DB::table('credito')
                  ->join('credito_garantia','credito_garantia.idcredito','credito.id')
                  ->join('credito_prendatario','credito_prendatario.id','credito.idcredito_prendatario')
                  ->where('credito_garantia.idgarantias',$value->idgarantias)
                  ->where('credito_garantia.idcredito',$credito->id)
                ...
                $lista_credito_garantia_aval_aval = DB::table('credito')
                  ...
                  ->join('credito_prendatario','credito_prendatario.id','credito.idcredito_prendatario')
                  ->where('credito_garantia.idgarantias',$value->idgarantias)
                  ->where('credito_garantia.idcliente',$idcredito_aval_idaval)
                  ->where('credito_garantia.tipo','CLIENTE')
                  ->where('credito.idestadocredito',1)
                  ->select(
                      'credito.id as idcredito',
                  ...
                  ->union($lista_credito_garantia_aval_propuesta)
                  ->union($lista_credito_garantia_aval_propio)
                  ->get();
              }else{
                $lista_credito_garantia_aval_propuesta = DB::table('credito')
                  ->join('credito_garantia','credito_garantia.idcredito','credito.id')
                ...
                //dump($lista_credito_garantia_aval_propuesta->get());
    ...
    public function update(Request $request, $idtienda, $id)
    {
        ...
        elseif($request->input('view') == 'credito_garantia') {
            ...
            DB::table('credito')->whereId($id)->update([
                ...
                'saldo_pendientepago'       => $request->input('total_pagar'),
            ...
            foreach($select_garantias_aval as $value){
              ...
              if($garantias){
                  ...
                  $garantias_descripcion = $garantias->descripcion;
                  $garantias_serie_motor_partida = $garantias->serie_motor_partida;
                  $garantias_chasis = $garantias->chasis!=''?$garantias->chasis:'';
                  $garantias_modelo_tipo = $garantias->modelo_tipo;
                  $garantias_otros = $garantias->otros;
                  $garantias_color = $garantias->color;
                  $garantias_fabricacion = $garantias->fabricacion;
