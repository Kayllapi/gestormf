    ...
    <div class="custom-form">
        ...
        <div id="cont-form-facturacionnotacredito" style="display:none;">
         <form class="js-validation-signin px-30" 
                  action="javascript:;" 
                  onsubmit="callback({
                    route: 'backoffice/tienda/nuevosistema/{{ $tienda->id }}/facturacionnotacredito',         
                    method: 'POST',
                    data:{
                        view: 'registrar',
...
@if($configuracion['idempresapordefecto']!=null)
    $("#idagencia").select2({
        ...
        $.ajax({
                url:"{{url('backoffice/tienda/nuevosistema/'.$tienda->id.'/facturacionnotacredito/show-selecionarserie')}}",
                type:'GET',
                data: {
...
$('#facturador_correlativo').keyup( function(e) {
    ...
    function cargarventa_boletafactura(idagencia, facturador_serie, facturador_correlativo){
        ...
        $.ajax({
            url:  "{{url('backoffice/tienda/nuevosistema/'.$tienda->id.'/facturacionnotacredito/show-seleccionarboletafactura')}}",
            type: 'GET',
            data: {
                idagencia               : idagencia,
