...
$facturacionrespuesta= DB::table('s_facturacionrespuesta')
    ->where('s_facturacionrespuesta.s_idfacturacionresumendiario',$facturacionresumen->idusuarioresponsable)
  ...
  @if(isset($facturacionrespuesta))
    @if($facturacionrespuesta->estado=='ACEPTADA')
        ...
        <div class="custom-form" style="margin-bottom: 5px;">
            <a href="{{ url('public/backoffice/tienda/'.$tienda->id.'/sunat/produccion/resumendiario/'.$facturacionrespuesta->nombre.'.xml') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
            <i class="fa fa-download"></i> Descargar XML</a>
            <a href="{{ url('public/backoffice/tienda/'.$tienda->id.'/sunat/produccion/resumendiario/R-'.$facturacionrespuesta->nombre.'.zip') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
    ...
    @else
        ...
        @endif
          <form class="js-validation-signin px-30" 
                action="javascript:;" 
                onsubmit="callback({
                  route: 'backoffice/tienda/nuevosistema/{{ $tienda->id }}/facturacionresumendiario/{{$facturacionresumen->id}}',
                  method: 'PUT',
                  data:{
        ...
        <div class="main-register fl-wrap">
            ...
            <div class="mx-modal-cuerpo" id="contenido-enviarcorreo">
              ...
              <form class="js-validation-signin px-30" 
                  action="javascript:;" 
                  onsubmit="callback({
                    route: 'backoffice/tienda/nuevosistema/{{ $tienda->id }}/facturacionresumendiario/0',
                    method: 'PUT',
