 <div id="carga-transferencia">
   <form class="form-tranferenciaproducto" @include('app.nuevosistema.submit',[
         'method'=>'PUT',
        'view'=>'edit',
        'id' => $productotransferencia->id,
        ...
        'function'=> [
          ...
          <div class="row">
                ...
                <table class="table" id="tabla-contenido">
                      <thead class="thead-dark">
                          ...
                          <tr>
                              ...
                              <td colspan="{{ $configuracion['estadostock']==1?'5':'4'}}" class="mx-td-input">
            ...
            <a href="javascript:;" onclick="$('.form-tranferenciaproducto').submit()" class="btn mx-btn-post">Guardar Cambios</a>
...
}).val({{$productotransferencia->idestadotransferencia}}).trigger('change'); 
...
$('#idtiendadestino').select2({
 ...
 $("#idproducto").select2({
  ...
  function buscarcodigo(pthis){
    if($(pthis).val()!=''){
        $.ajax({
            url:"{{url('backoffice/tienda/sistema/'.$tienda->id.'/productotransferencia/showseleccionarproductocodigo')}}",
            type:'GET',
            data: {
    ...
    @foreach($detalletransferencia as $value) 
        agregarproducto(
          {{$value->idproducto}},
          '{{$value->producodigoimpresion}}',
          '{{$value->productonombre}}',
          '{{productosaldo($tienda->id,$value->idproducto)['stock']}}',
   ...
   // funcion agregarproducto(), agregando en una nueva fila el producto
    function agregarproducto(idproducto,codigo,nombre,stock,idunidadmedida,unidadmedidanombre,cantidad=1,motivo=''){
        $("#codigoproducto").val('');
        $("#idproducto").html('');
        ...
        var nuevaFila='<tr id="'+num+'" idproducto="'+idproducto+'" nombreproducto="'+codigo+' - '+nombre+'" style="'+style+'">';
            ...
            nuevaFila+='<td class="with-form-control"><input id="productCant'+num+'" type="number" value="'+cantidad+'"></td>';
            nuevaFila+='<td class="with-form-control"><input id="productMotivo'+num+'" type="text" value="'+motivo+'"></td>'; 
