...
<div id="carga-venta">
    <div class="profile-edit-container">
        <div class="custom-form">
          @if($configuracion['nivelventa']==1)
          <div class="box" style="width: 300px;">
            <input class="Switcher__checkbox sr-only" id="idestado" type="checkbox" <?php echo $configuracion['estadoventa']==3? 'checked="checked"':'' ?>>
          ...
          @elseif($configuracion['nivelventa']==2)
          <div class="box" style="width: 310px;">
            <input class="Switcher__checkbox sr-only" id="idestado" type="checkbox" <?php echo $configuracion['estadoventa']==2? 'checked="checked"':'' ?>>
            <label class="Switcher" for="idestado">
              <div class="Switcher__trigger" data-value="Realizar Pedido"></div>
          ...
          <div class="tabs-container" id="tab-carritocompra">
              ...
              <div class="tab">
                  <div id="tab-carritocompra-0" class="tab-content" style="display: block;">
                      ...
                      <div id="comida_cont_codigo">
                          <div class="row">
                              <div class="col-md-4">
                              </div>
                              <div class="col-md-4">
                                  <input type="number" id="comida_codigo_pedido" placeholder="Código de Pedido" style="text-align: center;font-size: 14px;border-radius: 20px;">
                              </div>
                          </div>
                      </div>
                      <div id="comida_cont_mesa" style="display:none;">
                          <div class="mensaje-success" style="font-size: 20px;">
                            <i class="fa fa-table"></i> <span id="comida-cont-nombre-mesa"></span> - 
                            <a href="javascript:;" onclick="comida_limpiarpedido()" style="padding: 6px 20px;border-radius: 25px;color: #fff;cursor: pointer;background-color: #df1355;font-size: 14px;top: -2px;"><i class="fa fa-close"></i> Limpiar</a>
                          </div>
                        ...
                        <table class="table" id="tabla-contenido">
                            <thead class="thead-dark">
                              ...
                              <tr>
                                  ...
                                  <td colspan="{{$configuracion['estadostock']==1?'6':'5'}}" class="mx-td-input">
...
<script> 
  /* -------------- COMIDA ------------------*/
  $('#comida_codigo_pedido').keyup( function(e) {
      if(e.keyCode == 13 && $('#comida_codigo_pedido').val()!=''){
          comida_buscarcodigopedido();
      }
  })
  function comida_buscarcodigopedido(){
      load('#comida_load_buscarcodigopedido'); 
      $.ajax({
          url:"{{url('backoffice/tienda/sistema/'.$tienda->id.'/venta/show-comida-seleccionarproducto')}}",
          type:'GET',
          data:{
              codigopedido : $("#comida_codigo_pedido").val(),
          },
          success: function (respuesta){
            $("#comida_load_buscarcodigopedido").html(''); 
            if(respuesta['pedido']!=null){
                if(respuesta['pedido'].idestado==1){
                    $("#comida_load_buscarcodigopedido").html('<div class="mensaje-info">'+respuesta['mensaje']+'</div>');  
                }
                else if(respuesta['pedido'].idestado==2){
                    $("#comida_cont_codigo").css('display','none');
                ...
                else if(respuesta['pedido'].idestado==3){
                    $("#comida_load_buscarcodigopedido").html('<div class="mensaje-warning">'+respuesta['mensaje']+'</div>');  
                }
                else if(respuesta['pedido'].idestado==4){
                    $("#comida_load_buscarcodigopedido").html('<div class="mensaje-danger">'+respuesta['mensaje']+'</div>');         
                }
  ...
  function comida_limpiarpedido(){
      $("#comida_codigo_pedido").val('');
      $("#comida_idpedido").val('');
      $("#comida_cont_codigo").css('display','block');
      $("#comida_cont_mesa").css('display','none');
      limpiarventa();
      $("#comida_codigo_pedido").select();
  }
  /* -------------- FIN COMIDA ------------------*/
...
function limpiarventa(){ 
    ...
    @if($configuracion_facturacion['idclientepordefecto']!=null)
    $("#idcliente").html('<option value="{{ $configuracion_facturacion['idclientepordefecto'] }}">{{ $configuracion_facturacion['clientepordefecto'] }}</option>');
    $.ajax({
        url:"{{url('backoffice/tienda/sistema/'.$tienda->id.'/venta/showseleccionarusuario')}}",
        type:'GET',
        data: {
            idusuario : {{ $configuracion_facturacion['idclientepordefecto'] }}
        },
        success: function (respuesta){
          $('#direccion').val(respuesta['usuario'].direccion);
          if(respuesta['usuario'].idubigeo!=0){
    ...
    @endif
    @if($configuracion_facturacion['idempresapordefecto']!=null)
    $("#idagencia").val({{ $configuracion_facturacion['idempresapordefecto'] }}).trigger('change');
    @endif
    @if($configuracion_facturacion['idcomprobantepordefecto']!=null)
    $("#idcomprobante").val({{ $configuracion_facturacion['idcomprobantepordefecto'] }}).trigger("change");
    @endif
