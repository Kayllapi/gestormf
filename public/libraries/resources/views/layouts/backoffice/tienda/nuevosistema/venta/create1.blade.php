...
<div id="cont-ventacomida">
<div class="profile-edit-container">
    <div class="statistic-container fl-wrap">
     @for($i = 1; $i <= $configuracion['ventacomida_numeromesa']; $i++)
     <a href="javascript:;" class="statistic-item-wrap" onclick="seleccionar_mesa('{{ str_pad($i, 2, "0", STR_PAD_LEFT) }}')">
     <div class="statistic-item gradient-bg fl-wrap">
         <i class="fa fa-utensils"></i>
            <div class="statistic-item-numder" style="font-size: 40px;">{{ str_pad($i, 2, "0", STR_PAD_LEFT) }}</div>
        </div>
     </a>
     @endfor
    </div>  
    <input type="hidden" id="ventacomida_numeromesa">
</div>
</div>
@endif
<div id="cont-ventacomidaestado" <?php echo $configuracion['ventacomida_estado']==1?'style="display:none;"':'' ?>>
<div id="carga-venta">
    <div class="profile-edit-container">
        <div class="custom-form">
          @if($configuracion['ventacomida_estado']==1)
          <a href="javascript:;" id="resultado-numeromesa" onclick="cambiar_mesa()">---</a>
          @endif
          @if($configuracion['venta_nivelventa']==1)
          <div class="box" style="width: 300px;">
            <input class="Switcher__checkbox sr-only" id="idestado" type="checkbox" <?php echo $configuracion['venta_estadoventa']==3? 'checked="checked"':'' ?>>
          ...
          @elseif($configuracion['venta_nivelventa']==2)
          <div class="box" style="width: 310px;">
            <input class="Switcher__checkbox sr-only" id="idestado" type="checkbox" <?php echo $configuracion['venta_estadoventa']==2? 'checked="checked"':'' ?>>
            <label class="Switcher" for="idestado">
              <div class="Switcher__trigger" data-value="Realizar Pedido"></div>
                        ...
                        <table class="table" id="tabla-contenido">
                            <thead class="thead-dark">
                              ...
                              <tr>
                                  ...
                                  <td colspan="{{$configuracion['venta_estadostock']==1?'6':'5'}}" class="mx-td-input">
                  ...
                  <div id="tab-carritocompra-2" class="tab-content" style="display: none;">
                      ...
                      <div class="row">
                         <div class="col-md-6">
                            ...
                            @if($configuracion['venta_idclientepordefecto']!=null)
                                <div class="row">
                                   <div class="col-md-9">
                                      <select id="idcliente">
                                          <option value="{{ $configuracion['venta_idclientepordefecto'] }}">{{ $configuracion['venta_clientepordefecto'] }}</option>
                                      </select>
                                   </div>
                                   <div class="col-md-3">
                                ...
                                <input type="text" id="direccion" value="{{$configuracion['venta_clientedireccionpordefecto']}}"/>
                                <label>Facturación - Ubicación (Ubigeo)</label>
                                <select id="idubigeo">
                                    <option value="{{ $configuracion['venta_clienteidubigeopordefecto'] }}">{{ $configuracion['venta_clienteubigeopordefecto'] }}</option>
                                </select>
                            @else
                                <div class="row">
                                   <div class="col-md-9">
                  ...
                  <div id="tab-carritocompra-3" class="tab-content" style="display: none;">
                      <div class="row">
                        ...
                        <div class="col-md-4">
                          ...
                          <div id="cont-montorecibido" <?php echo ($configuracion['venta_estadoventa']==1 or $configuracion['venta_estadoventa']==2)? 'style="display:none;"':'' ?>>
                              <label>Total </label>
                      ...
                      <a href="javascript:;" onclick="registrar_venta()" id="cont-btnventa" class="btn  big-btn  color-bg flat-btn mx-realizar-pago">
                          @if($configuracion['venta_estadoventa']==1) 
                            Registrar Pedido
                          @elseif($configuracion['venta_estadoventa']==2) 
                            Confirmar Pedido
                          @elseif($configuracion['venta_estadoventa']==3) 
                            Realizar Venta
                          @endif      
                      </a>
                  </div>
...
@if($configuracion['ventacomida_estado']==1)
    function seleccionar_mesa(numeromesa){
        $('#ventacomida_numeromesa').val(numeromesa);
        $('#cont-ventacomida').css('display','none');
        $('#cont-ventacomidaestado').css('display','block');
        $('#resultado-numeromesa').html('Número de Mesa: '+numeromesa);
    }
    function cambiar_mesa(){
        $('#ventacomida_numeromesa').val('');
        $('#cont-ventacomida').css('display','block');
        $('#cont-ventacomidaestado').css('display','none');
        $('#resultado-numeromesa').html('---');
    }
@endif  
...
@if($configuracion['venta_idempresapordefecto']!=null)
    $("#idagencia").select2({
        placeholder: "--  Seleccionar --",
        minimumResultsForSearch: -1
    }).val({{ $configuracion['venta_idempresapordefecto'] }}).trigger("change");    
@else
    $("#idagencia").select2({
        placeholder: "--  Seleccionar --",
        minimumResultsForSearch: -1
...
@if($configuracion['venta_idcomprobantepordefecto']!=null)
    $("#idcomprobante").select2({
        placeholder: "--  Seleccionar --",
        minimumResultsForSearch: -1
    }).val({{ $configuracion['venta_idcomprobantepordefecto'] }}).trigger("change");   
@else
    $("#idcomprobante").select2({
        placeholder: "--  Seleccionar --",
        minimumResultsForSearch: -1
...
@if($configuracion['venta_idtipoentregapordefecto']!=null)
    $("#idtipoentrega").select2({
        ...
        calcularmonto();
    }).val({{ $configuracion['venta_idtipoentregapordefecto'] }}).trigger("change");
@else
    $("#idtipoentrega").select2({
        placeholder: "--  Seleccionar --",
        minimumResultsForSearch: -1
...
function limpiarventa(){ 
    ...
    @if($configuracion['venta_idclientepordefecto']!=null)
    $("#idcliente").html('<option value="{{ $configuracion['venta_idclientepordefecto'] }}">{{ $configuracion['venta_clientepordefecto'] }}</option>');
    $.ajax({
        url:"{{url('backoffice/tienda/sistema/'.$tienda->id.'/venta/showseleccionarusuario')}}",
        type:'GET',
        data: {
            idusuario : {{ $configuracion['venta_idclientepordefecto'] }}
        },
        success: function (respuesta){
          $('#direccion').val(respuesta['usuario'].direccion);
          if(respuesta['usuario'].idubigeo!=0){
    ...
    @endif
    @if($configuracion['venta_idempresapordefecto']!=null)
    $("#idagencia").val({{ $configuracion['venta_idempresapordefecto'] }}).trigger('change');
    @endif
    @if($configuracion['venta_idcomprobantepordefecto']!=null)
    $("#idcomprobante").val({{ $configuracion['venta_idcomprobantepordefecto'] }}).trigger("change");
    @endif
