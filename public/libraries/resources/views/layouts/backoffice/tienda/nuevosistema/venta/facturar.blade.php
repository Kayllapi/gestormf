...
<?php 
$s_facturacionboletafactura = DB::table('s_facturacionboletafactura')
    ...
    ->select(
        ...
        's_facturacionrespuesta.nombre as nombrearchivo',
        's_facturacionrespuesta.estado as estado',
        's_facturacionrespuesta.mensaje as mensaje'
    )
...
@if(isset($s_facturacionboletafactura))
    @if($s_facturacionboletafactura->estado=='ACEPTADA')
        <div class="mensaje-info">
          {{$s_facturacionboletafactura->mensaje}}<br>
        </div>
        <div class="custom-form" style="margin-bottom: 5px;">
            <a href="{{ url('public/backoffice/tienda/'.$tienda->id.'/sunat/produccion/boletafactura/'.$s_facturacionboletafactura->nombrearchivo.'.xml') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
            <i class="fa fa-download"></i> Descargar XML</a>
            <a href="{{ url('public/backoffice/tienda/'.$tienda->id.'/sunat/produccion/boletafactura/R-'.$s_facturacionboletafactura->nombrearchivo.'.zip') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
            <i class="fa fa-download"></i> Descargar CDR</a>
            <a href="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/facturacionboletafactura/'.$s_facturacionboletafactura->id.'/edit?view=ticketpdf') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
        ...
        <iframe src="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/facturacionboletafactura/'.$s_facturacionboletafactura->id.'/edit?view=ticketpdf') }}#zoom=130" frameborder="0" width="100%" height="600px"></iframe>
    @else
        @if($s_facturacionboletafactura->estado=='OBSERVACIONES')
          <div class="mensaje-info">
            El Comprobante tiene Observaciones:<br>
            {{$s_facturacionboletafactura->mensaje}}<br>
            <b>¿Deseas reenviar el comprobante?</b><br>
          </div>
        @elseif($s_facturacionboletafactura->estado=='RECHAZADA')
          <div class="mensaje-info">
            El Comprobante fue rechazado:<br>
            {{$s_facturacionboletafactura->mensaje}}<br>
            <b>¿Deseas reenviar el comprobante?</b><br>
          </div>
        @elseif($s_facturacionboletafactura->estado=='EXCEPCION')
          <div class="mensaje-info">
          ...
          <!-- Seccion mostrando el total, subtotal, igv -->
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    $igv = ($configuracion_facturacion['igv']/100)+1;
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">IGV ({{$configuracion_facturacion['igv']}}%)</label>
                        <div class="col-sm-9">
