...
<table class="table" id="tabla-contenido">
    ...
    <tbody>
        ...
        <tr>
          ...
          <td style="background-color: #28a745;color: #fff;padding: 5px;">{{$moneda_soles->simbolo}} {{$value->montoasignar}}</td>
          <td style="background-color: #28a745;color: #fff;"></td>
          @elseif($value->config_sistema_moneda_usar==2)
          <td style="background-color: #28a745;color: #fff;padding: 5px;">{{$moneda_dolares->simbolo}} {{$value->montoasignar_dolares}}</td>
          <td style="background-color: #28a745;color: #fff;"></td>
          ...
          @endif
          @if($value->config_sistema_moneda_usar==1)
          <td style="background-color: #007bff;color: #fff;padding: 5px;">{{$moneda_soles->simbolo}} {{$value->montocierre_recibido}}</td>
          <td style="background-color: #007bff;color: #fff;"></td>
          @elseif($value->config_sistema_moneda_usar==2)
          <td style="background-color: #007bff;color: #fff;padding: 5px;">{{$moneda_dolares->simbolo}} {{$value->montocierre_recibido_dolares}}</td>
          <td style="background-color: #007bff;color: #fff;"></td>
          @elseif($value->config_sistema_moneda_usar==3)
          <td style="background-color: #007bff;color: #fff;padding: 5px;">{{$moneda_soles->simbolo}} {{$value->montocierre_recibido}}</td>
          <td style="background-color: #007bff;color: #fff;padding: 5px;">{{$moneda_dolares->simbolo}} {{$value->montocierre_recibido_dolares}}</td>
          ...
          <td>
            <div class="header-user-menu menu-option" id="menu-opcion">
                ...
                <ul>
                    @if($value->idestadoaperturacierre==1 && $value->idusersresponsable==Auth::user()->id)
                    <li><a href="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/cajaapertura/'.$value->id.'/edit?view=editar') }}"><i class="fa fa-edit"></i> Confirmar</a></li>
                    <li><a href="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/cajaapertura/'.$value->id.'/edit?view=eliminar') }}"><i class="fa fa-trash"></i> Eliminar</a></li>
                    @elseif($value->idestadoaperturacierre==2 && $value->idusersresponsable==Auth::user()->id && $value->idusersrecepcion!=Auth::user()->id && $value->fechaconfirmacion=='')
