...
<div class="list-single-main-wrapper fl-wrap">
    <div class="breadcrumbs gradient-bg fl-wrap">
      <span>Editar Carrito de compra</span>
...
<form class="js-validation-signin px-30" 
      action="javascript:;" 
      onsubmit="callback({
        route: 'backoffice/tienda/sistema/{{ $tienda->id }}/carritocompra/{{ $carritocompra->id }}',
        method: 'PUT',
        data:{
            view: 'editar',
                        ...
                        <div class="row">
                           <div class="col-md-6">
                              ...
                              <div class="row">
                                 <div class="col-md-9">
                                    <select id="idcliente" >
                                        <option value="{{ $carritocompra->s_idusuariocliente }}">{{ $carritocompra->clienteidentificacion }} - {{ $carritocompra->clienteapellidos }}, {{ $carritocompra->clientenombre }}</option>
                                    </select>
                                 </div>
                                 <div class="col-md-3">
                              ...
                              <input type="text" value="{{ $carritocompra->clientedireccion }}" id="direccion"/>
                           </div>
                           <div class="col-md-6">
                              <label>Ubicación (Ubigeo)</label>
                              <select id="idubigeo">
                                  <option value="{{ $carritocompra->idubigeo!=0?$carritocompra->idubigeo:'' }}">{{ $carritocompra->ubigeonombre }}</option>
                              </select>
                           </div>
                         </div>
                      </div>
                        ...
                        <div class="custom-form">
                          <div class="row">
                            <div class="col-md-6">
                              ...
                              <div class="row">
                                  <div class="col-md-6">
                                      <input type="date" value="{{ $carritocompra!=''?$carritocompra->envio_fecha:'' }}" id="delivery_fecha">
                                  </div>
                                  <div class="col-md-6">
                                      <input type="time" value="{{ $carritocompra!=''?$carritocompra->envio_hora:'' }}" id="delivery_hora">
                                  </div>
                              </div>
                              </div>
                              <label>Nombre de persona a entregar *</label>
                              <input type="text" value="{{ $carritocompra!=''?$carritocompra->envio_nombre:'' }}" id="delivery_pernonanombre">
                              <label>Número de celular de entrega *</label>
                              <input type="text" value="{{ $carritocompra!=''?$carritocompra->envio_telefono:'' }}" id="delivery_numerocelular">
                              <label>Dirección de entrega *</label>
                              <input type="text" value="{{ $carritocompra!=''?$carritocompra->envio_direccion:'' }}" id="delivery_direccion">
                            </div>
                            <div class="col-md-6">
                              <label>Ubicación de entrega (Referencia) *</label>
                              <div id="singleMap"></div>
                              <input type="hidden" value="{{ $carritocompra!=''?$carritocompra->mapa_ubicacion_lat:'' }}" id="mapa_ubicacion_lat"/>
                              <input type="hidden" value="{{ $carritocompra!=''?$carritocompra->mapa_ubicacion_lng:'' }}" id="mapa_ubicacion_lng"/>
                            </div>
                          </div>
                        </div>
                      </div>
...
}).val({{ $carritocompra->s_idestadoenvio }}).trigger("change");
...
$("#idproducto").select2({
    ...
    $.ajax({
        url:"{{url('backoffice/tienda/sistema/'.$tienda->id.'/venta/')}}/"+e.currentTarget.value,
        type:'GET',
        data: {
            view : 'addproducto'
        },
        success: function (respuesta){
          if(respuesta["datosProducto"]!=null){
            agregarproducto(
              ...
              respuesta["datosProducto"].codigo,
              respuesta["datosProducto"].nombre,
              respuesta["stock"],
              respuesta["datosProducto"].precioalpublico,
              '0.00'
            );
          }
        }
...
function buscarcodigo(pthis){
    $.ajax({
        url:"{{url('backoffice/tienda/sistema/'.$tienda->id.'/venta/')}}/"+$(pthis).val(),
        type:'GET',
        data: {
            view : 'addproductocodigo'
        },
        success: function (respuesta){
          if(respuesta["datosProducto"]!=null){
            agregarproducto(
...
@foreach($carritocompradetalles as $value)
agregarproducto(
  '{{ $value->s_idproducto }}',
  '{{ $value->productocodigo }}',
  '{{ $value->productonombre }}',
  '{{ stock_producto($value->s_idproducto)['stock'] }}',
  '{{ $value->cantidad }}',
  '{{ $value->preciounitario }}',
  '{{ $value->descuento }}'
);
@endforeach
function agregarproducto(idproducto,codigo,nombre,stock,cantidad,precioalpublico,descuento){
      $("#codigoproducto").val('');
      $("#idproducto").html('');
      ...
      var nuevaFila='<tr id="'+num+'" idproducto="'+idproducto+'" style="'+style+'">';
          ...
          nuevaFila+='<td class="mx-td-input"><input id="productCant'+num+'" type="number" value="'+cantidad+'" onkeyup="calcularmonto()"></td>';
...
<script>
    function singleMap() {
        var myLatLng = {
            lat: {{ $carritocompra!=''?$carritocompra->mapa_ubicacion_lat:'' }},
            lng: {{ $carritocompra!=''?$carritocompra->mapa_ubicacion_lng:'' }},
        };
