...
<div class="list-single-main-wrapper fl-wrap">
    <div class="breadcrumbs gradient-bg fl-wrap">
      <span>REGISTRAR CONCEPTO MOVIMIENTO</span>
...
<form class="js-validation-signin px-30" 
      action="javascript:;" 
      onsubmit="callback({
        route: 'backoffice/tienda/sistema/{{ $tienda->id }}/conceptomovimiento',
        method: 'POST',
        data:{
            view: 'registrar',
            idtienda: '{{ $tienda->id }}'
             ...
             <div class="col-md-6">
                ...
                <select id="tipo">
                  @foreach($movementType as $value)
                    <option value="{{$value->id}}" <?=$value->id == 1 ? 'selected' : '' ?> > {{ $value->nombre }} </option>
                  @endforeach
                </select>
             </div>
           </div>
