...
<div class="list-single-main-wrapper fl-wrap">
    <div class="breadcrumbs gradient-bg fl-wrap">
      <span>Ticket Nota de Débito</span>
  ...
  @if(isset($facturacionrespuesta))
    @if($facturacionrespuesta->estado=='ACEPTADA')
        ...
        <div class="custom-form" style="margin-bottom: 5px;">
            <a href="{{ url('public/backoffice/tienda/'.$tienda->id.'/sunat/produccion/notadebito/'.$facturacionrespuesta->nombre.'.xml') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
            <i class="fa fa-download"></i> Descargar XML</a>
            <a href="{{ url('public/backoffice/tienda/'.$tienda->id.'/sunat/produccion/notadebito/R-'.$facturacionrespuesta->nombre.'.zip') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
        ...
        <iframe id="content-ticketpdf" style="display: block" src="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/facturacionnotadebito/'.$facturacionnotadebito->id.'/edit?view=ticketpdf') }}#zoom=130" frameborder="0" width="100%" height="600px"></iframe>
        <iframe id="content-a4pdf" style="display: none" src="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/facturacionnotadebito/'.$facturacionnotadebito->id.'/edit?view=a4pdf') }}#zoom=130" frameborder="0" width="100%" height="600px"></iframe>
    @else
        ...
        @endif
          <form class="js-validation-signin px-30" 
                action="javascript:;" 
                onsubmit="callback({
                  route: 'backoffice/tienda/sistema/{{ $tienda->id }}/facturacionnotadebito/{{$facturacionnotadebito->id}}',
                  method: 'PUT',
                  data:{
        ...
        <div class="main-register fl-wrap">
            ...
            <div class="mx-modal-cuerpo" id="contenido-enviarcorreo">
              ...
              <form class="js-validation-signin px-30" 
                  action="javascript:;" 
                  onsubmit="callback({
                    route: 'backoffice/tienda/sistema/{{ $tienda->id }}/facturacionnotadebito/0',
                    method: 'PUT',
                    carga: '#mx-carga-enviarcorreo',
                    data:{
                        view: 'enviarcorreo',
                        idfacturacionnotadebito: {{$facturacionnotadebito->id}}
                    }
                },
                function(resultado){
