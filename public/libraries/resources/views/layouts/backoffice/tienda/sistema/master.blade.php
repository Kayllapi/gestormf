...
<div class="profile-edit-page-header">
    ...
    @if(Auth::user()->idtienda!=0)
        ...
        @elseif($caja['resultado']=='ABIERTO')
           <a href="javascript:;" class="mx-alert-caja activo"><i class="fa fa-tags"></i> Caja Activa 
             @if(configuracion($tienda->id,'prestamo_tipocierrecaja')['valor']==1)
             ({{$moneda_soles->simbolo}} {{ efectivo($tienda->id,$caja['apertura']->id,1)['total'] }} - {{$moneda_dolares->simbolo}} {{ efectivo($tienda->id,$caja['apertura']->id,2)['total'] }})
        ...
        @else
          ...
          @endforeach
            <?php /*$usuario = DB::table('users')
                ...
                ->select('users.*','role_user.cargo as permiso')
                ->limit(1)
                ->first();*/ 
            $cant_alerta = alerta_fechavencimiento($tienda->id)['cantidad'];
            ?>
            @if($cant_alerta>0)
            <li class="li-alert-sistema">
              <a href="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/inicio/1/edit?view=productovencido') }}">
              <i class="fa fa-bell"></i><span class="alert-sistema">{{$cant_alerta}}</span>&nbsp;
              </a>
            </li>
            @endif
            <li>
