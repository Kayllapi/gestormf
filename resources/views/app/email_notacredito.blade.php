	...
	<tbody><tr>
		...
		<table border="0" cellpadding="0" cellspacing="0" style="max-width:400px;" width="100%" class="wrapperBody">
          ...
          <!-- Space -->
                ...
                <div style="font-family: Helvetica;font-weight: bold;font-size: 12px;width: 100%;text-align: center;color: #000;margin-top:20px;">
                      @if($facturacionnotacredito->agencialogo!='')
                          <img class="logo" src="{{ url('public/backoffice/tienda/'.$tienda->id.'/sistema/'.$facturacionnotacredito->agencialogo) }}" height=50px><br>
                      @endif
                      {{ strtoupper($facturacionnotacredito->emisor_nombrecomercial) }}
                      <br>
                      RUC:  {{ $facturacionnotacredito->emisor_ruc }}<br>
                            {{ strtoupper($facturacionnotacredito->emisor_direccion) }}<br>
                            {{ strtoupper($facturacionnotacredito->emisor_departamento.'/'.$facturacionnotacredito->emisor_provincia.'/'.$facturacionnotacredito->emisor_distrito) }}<br><br>

                      <div style="text-align: left;color: #000;">   
                        NOTA DE CRÉDITO: {{$facturacionnotacredito->notacredito_serie}}-{{ str_pad($facturacionnotacredito->notacredito_correlativo, 8, "0", STR_PAD_LEFT) }}<br>
                        EMISIÓN: {{ date_format(date_create($facturacionnotacredito->notacredito_fechaemision),"d/m/Y h:i:s A") }}<br>
                        @if($facturacionnotacredito->	notacredito_tipomoneda=='PEN')
                        MONEDA: SOLES<br>
                        @else($facturacionnotacredito->	notacredito_tipomoneda=='USD')
                        MONEDA: DOLARES<br>
                        @endif
                        CLIENTE: {{ strtoupper($facturacionnotacredito->cliente_razonsocial) }}<br>
                        DNI/RUC: {{ $facturacionnotacredito->cliente_numerodocumento }}<br>
                        DIRECCIÓN: {{ strtoupper($facturacionnotacredito->cliente_direccion) }}<br>
                        {{ strtoupper($facturacionnotacredito->cliente_departamento.'/'.$facturacionnotacredito->cliente_provincia.'/'.$facturacionnotacredito->cliente_distrito) }}<br>
                        RESPONSABLE: {{ strtoupper($facturacionnotacredito->responsablenombre) }}<br><br>
                    </div>
                    <?php 
                    $facturacionboletafactura = DB::table('s_facturacionboletafactura')
                        ->where('s_facturacionboletafactura.id',$facturacionnotacredito->idfacturacionboletafactura)
                        ->first(); 
                    ?>
                    ...
                    <div style="text-align: left;color: #000;">
                        ...
                        MOTIVO: {{strtoupper($facturacionnotacredito->notacredito_descripcionmotivo) }}
                    </div>
                    ...
                    <tbody>
                      @foreach($notacreditodetalle as $value)
                      <tr>
                     ...
                     <tr>
                        ...
                        <td style="white-space: nowrap;text-align: right;font-family: Courier;color: #000;">
                          {{ $facturacionnotacredito->notacredito_valorventa }}
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2" style="text-align: right;font-family: Courier;color: #000;">IGV:</td>
                        <td style="white-space: nowrap;text-align: right;font-family: Courier;color: #000;">
                          {{ $facturacionnotacredito->notacredito_totalimpuestos }}
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2" style="text-align: right;font-family: Courier;color: #000;">TOTAL:</td>
                        <td style="white-space: nowrap;text-align: right;font-family: Courier;color: #000;">
                          {{ $facturacionnotacredito->notacredito_montoimpuestoventa }}
                        </td>
                      </tr>
                      <tr>
                        <td colspan="3" style="text-align: center;height:5px;font-family: Courier;color: #000;"><div style="border-top: 1px dashed #31353d;width:100%;"></div></td>
                      ...
                      <tr>
                        <td colspan="3" style="text-align: center;height:5px;font-family: Courier;color: #000;">{{ $facturacionnotacredito->leyenda_value }}</td>
                      </tr>
                      <tr>
                        <td colspan="3" style="text-align: center;height:5px;font-family: Courier;color: #000;"><div style="border-top: 1px dashed #31353d;width:100%;"></div></td>
                      </tr>
