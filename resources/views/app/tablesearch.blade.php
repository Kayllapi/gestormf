...
@foreach($searchs as $value)
    ...
    <form action="javascript:;" onsubmit="search_table()">
      ...
      @if(count($list)>1)
        @if($list[0]=='date')
          <input type="date" style="padding: 7.5px;" value="{{ isset($_GET[$list[1]])?$_GET[$list[1]]:'' }}" id="search-table-{{ $list[1] }}" class="form-control" placeholder="Buscar..." onchange="search_table()">        
          <?php $inputvalue = $list[1]; ?>
        @elseif($list[0]=='select')
          <?php $listdata = explode('/',$list[1]); ?>
          <select id="search-table-{{ $listdata[0] }}" class="form-control" onchange="search_table()" style="padding: 10px;border-radius: 5px;width: 100%;border: 1px solid #d3d8de;">
      ...
      <input type="text" value="{{ isset($_GET[$value])?$_GET[$value]:'' }}" id="search-table-{{ $value }}" class="form-control" placeholder="Buscar..." onkeyup="texto_mayucula(this)">
      @endif
    </form>
    <?php
