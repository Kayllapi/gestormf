...
<script>
  ...
  function lista_asignacioncapital(id){
    ...
    $.ajax({
      url:"{{url('backoffice/0/asignacioncapital/show_table')}}",
      type:'GET',
      data:{
  ...
  function lista_saldocapitalasignado(id){
    $.ajax({
      url:"{{url('backoffice/0/asignacioncapital/show_saldocapitalasignado')}}",
      type:'GET',
      data:{
          fechainicio: $('#fechainicio').val(),
          fechafin: $('#fechafin').val(),
  ...
  function load_nuevo_asignacioncapital(){
    pagina({ route:"{{url('backoffice/'.$tienda->id.'/asignacioncapital/create?view=registrar')}}", result:'#form-result-giro'});
  }
  
    function validar(idasignacioncapital){
        modal({ route:"{{ url('backoffice/'.$tienda->id) }}/asignacioncapital/"+idasignacioncapital+"/edit?view=validar",  size: 'modal-sm' });
    }
  
   function reporte_saldocapitalasignado(idagencia){
      let url = "{{ url('backoffice/'.$tienda->id) }}/asignacioncapital/0/edit?view=reporte_saldocapitalasignado&idagencia="+idagencia;
  ...
  function show_data(e) {
      ...
      $(e).addClass('selected');
      pagina({ route:"{{url('backoffice')}}/{{$tienda->id}}/asignacioncapital/"+id+"/edit?view=editar", result:'#form-result-giro'});
   ...
   function exportar_pdf(){
      let url = "{{ url('backoffice/'.$tienda->id) }}/asignacioncapital/0/edit?view=exportar&fechainicio="+$('#fechainicio').val()+
          "&fechafin="+$('#fechafin').val();
      modal({ route: url,size:'modal-fullscreen' })
   }
   ...
   function recepcionar(){
      ...
      let url = "{{ url('backoffice/'.$tienda->id) }}/asignacioncapital/"+idasignacioncapital+"/edit?view=recepcionar";
      modal({ route: url })
   }
   function voucher(){
      let idasignacioncapital = $('#table-lista-asignacioncapital > tbody > tr.selected').attr('data-valor-columna');
      ...
      let url = "{{ url('backoffice/'.$tienda->id) }}/asignacioncapital/"+idasignacioncapital+"/edit?view=voucher";
      modal({ route: url })
   }
</script>
