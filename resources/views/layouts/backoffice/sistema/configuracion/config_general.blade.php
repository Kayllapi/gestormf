...
<script>
    ...
    @if(configuracion($tienda->id,'sistema_plantilla')['resultado']=='CORRECTO')
        $("#sistema_plantilla").select2({
            placeholder: "--  Seleccionar --",
            minimumResultsForSearch: -1
        }).val('{{ configuracion($tienda->id,'sistema_plantilla')['valor'] }}').trigger("change");    
    @else
        $("#sistema_plantilla").select2({
            placeholder: "--  Seleccionar --",
            minimumResultsForSearch: -1
