...
<script>
  ...
  function lista_credito(){
    ...
    $.ajax({
      url:"{{url('backoffice/0/controlaperturaopecaja/showtable')}}",
      type:'GET',
      data: {
    ...
    function cierre(){
        ...
        modal({ route:"{{ url('backoffice/'.$tienda->id) }}/controlaperturaopecaja/0/edit?view=cierre&fecha_corte="+fecha_corte,  size: 'modal-sm' });
    }
 
   function reporte(){
      let url = "{{ url('backoffice/'.$tienda->id) }}/controlaperturaopecaja/0/edit?view=reporte";
