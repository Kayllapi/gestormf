<div class="modal-header">
  <h5 class="modal-title">Créditos No Prendarias 
...
//   });
  ...
  function lista_garantias_cliente(id){
    $.ajax({
      url:"{{url('backoffice/0/creditoordinario/showcreditos')}}",
      type:'GET',
      data: {
          idcliente : id
      },
  ...
  function diasdegracia(){
    modal({route:"{{url('backoffice')}}/{{$tienda->id}}/creditoordinario/0/edit?view=diasdegracia"}); 
  }
  function show_data(e) {
    ...
    $(e).addClass('selected');
//     modal({route:"{{url('backoffice')}}/{{$tienda->id}}/creditoordinario/"+id+"/edit?view=editar"}); 
    pagina({ route:"{{url('backoffice')}}/{{$tienda->id}}/creditoordinario/"+id+"/edit?view=editar", result:'#form-credito-result'});
    
  }
  load_nuevo_credito();
  function load_nuevo_credito(){
    pagina({ route:"{{url('backoffice/'.$tienda->id.'/creditoordinario/create?view=registrar')}}", result:'#form-credito-result'});
  }
//   load_nuevo_credito();
