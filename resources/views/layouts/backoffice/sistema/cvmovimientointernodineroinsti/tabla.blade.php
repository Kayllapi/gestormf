...
<script>
  ...
  function load_nuevo_movimientointernodinero_retiro2(){
    pagina({ route:"{{url('backoffice/'.$tienda->id.'/cvmovimientointernodineroinsti/create?view=registrar_retiro2')}}", result:'#form-result-giro_retiro2'});
  }
  ...
  function load_nuevo_movimientointernodinero_deposito2(){
    pagina({ route:"{{url('backoffice/'.$tienda->id.'/cvmovimientointernodineroinsti/create?view=registrar_deposito2')}}", result:'#form-result-giro_deposito2'});
  }
  function show_data_deposito2(e) {
    let id = $(e).attr('data-valor-columna');
    $('tr.selected').removeClass('selected');
    $(e).addClass('selected');
    pagina({ route:"{{url('backoffice')}}/{{$tienda->id}}/cvmovimientointernodineroinsti/"+id+"/edit?view=editar_deposito2", result:'#form-result-giro_deposito2'});
  }
   function exportar_pdf(){
      let url = "{{ url('backoffice/'.$tienda->id) }}/cvmovimientointernodineroinsti/0/edit?view=exportar&fechainicio="+$('#fechainicio').val()+
          "&fechafin="+$('#fechafin').val();
      modal({ route: url,size:'modal-fullscreen' })
   }
  
  function valid_reporte(){
    modal({ route:"{{url('backoffice/'.$tienda->id.'/cvmovimientointernodineroinsti/0/edit?view=valid_reporte')}}",  size: 'modal-sm'  });  
  }
</script>
