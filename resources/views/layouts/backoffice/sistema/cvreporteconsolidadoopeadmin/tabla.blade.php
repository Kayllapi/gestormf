...
<script>
    ...
    function verpdf(){
        ...
        setTimeout(function(){
            $('#iframe_acta_aprobacion').attr('src',
                '{{ url('/backoffice/'.$tienda->id.'/cvreporteconsolidadoopeadmin/0/edit?view=pdf_reporte') }}'
                + '&corte=' + corte
                + '&idagencia=' + idagencia
                + '#zoom=100'
            );
    ...
    function validar_limites(){
        let corte = $('#corte').val();
        let idagencia = $('#idagencia').val();
        let url = "{{ url('backoffice/'.$tienda->id) }}/cvreporteconsolidadoopeadmin/0/edit?view=validar_limites";
        $.ajax({
            url: url,
            type:'GET',
            data:{
