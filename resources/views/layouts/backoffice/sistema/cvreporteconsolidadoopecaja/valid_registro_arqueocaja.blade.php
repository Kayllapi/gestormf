<form action="javascript:;" 
      onsubmit="callback({
          route: '{{ url('backoffice/'.$tienda->id.'/cvreporteconsolidadoopecaja/0') }}',
          method: 'PUT',
          data:{
      ...
      function(resultado){
          $('#idresponsable_registro').val(resultado.idresponsable);
          $('#idresponsable_registro_idpermiso').val($('#idresponsable :selected').attr('idpermiso'));
                
          submit_registro_arqueocaja();
          $('#modal-close-reporteconsolidadoopecaja-valid-arqueocaja').click();
      },this)">
    <div class="modal-header">
        <h5 class="modal-title">Validar</h5>
        <button type="button" class="btn-close" id="modal-close-reporteconsolidadoopecaja-valid-arqueocaja" data-bs-dismiss="modal" aria-label="Close"></button>
    </div>
