...
<form class="js-validation-signin px-30" 
      action="javascript:;" 
      onsubmit="callback({
        route: 'backoffice/tienda/sistema/{{ $tienda->id }}/facturacionboletafactura',
        method: 'POST',
        data:{
            view:      'registrar',
            idtienda:  '{{ $tienda->id }}',
                ...
                <table class="table" id="tabla-contenido">
                    <thead class="thead-dark">
                        ...
                        <tr>
                            ...
                            <td colspan="{{configuracion($tienda->id,'sistema_estadostock')['valor']==1?'5':'4'}}" class="mx-td-input">
...
<script>
  ...
  //Seleccionar y  Busqueda Producto
   $("#idproducto").select2({
    @include('app.select2_producto',[
        ...
        $.ajax({
            url:  "{{url('backoffice/tienda/sistema/'.$tienda->id.'/facturacionboletafactura/showseleccionarproducto')}}",
            type: 'GET',
            data: {
  ...
  function buscarcodigo(pthis){
    if($(pthis).val()!=''){
        $.ajax({
            url:"{{url('backoffice/tienda/sistema/'.$tienda->id.'/facturacionboletafactura/showseleccionarproductocodigo')}}",
            type:'GET',
            data: {
   ...
   function registrar_comprobante(){
    ...
    function calcularmonto(){
        ...
        subtotal = parseFloat(total/1.{{configuracion($tienda->id,'facturacion_igv')['resultado']=='CORRECTO'?configuracion($tienda->id,'facturacion_igv')['valor']:18}});
