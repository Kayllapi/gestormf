...
<div class="list-single-main-wrapper fl-wrap">
    <div class="breadcrumbs gradient-bg fl-wrap">
      <span>Ticket de Resumen Diario</span>
    ...
    @if($facturacionresumen->respuestaestado=='ACEPTADA' or $facturacionresumen->respuestacodigo == '2223' or $facturacionresumen->respuestacodigo == '0402')
        <div class="mensaje-info"><?php echo $facturacionresumen->respuestacodigo ?> - <?php echo $facturacionresumen->respuestamensaje ?></div>
        <div class="custom-form" style="margin-bottom: 5px;">
            <a href="{{ url('public/backoffice/tienda/'.$tienda->id.'/sunat/produccion/resumendiario/'.$facturacionresumen->respuestanombre.'.xml') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
            <i class="fa fa-download"></i> Descargar XML</a>
            <a href="{{ url('public/backoffice/tienda/'.$tienda->id.'/sunat/produccion/resumendiario/R-'.$facturacionresumen->respuestanombre.'.zip') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
            <i class="fa fa-download"></i> Descargar CDR</a>
            <a href="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/facturacionresumendiario/'.$facturacionresumen->id.'/edit?view=ticketpdf') }}" download class="btn big-btn color-bg flat-btn" style="float: left;margin-right: 5px;">
        ...
        <iframe src="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/facturacionresumendiario/'.$facturacionresumen->id.'/edit?view=ticketpdf') }}#zoom=130" frameborder="0" width="100%" height="600px"></iframe>
    @else
        @if($facturacionresumen->respuestaestado=='OBSERVACIONES')
          <div class="mensaje-info">
            El Comprobante tiene Observaciones:<br>
            <?php echo $facturacionresumen->respuestacodigo ?> - <?php echo $facturacionresumen->respuestamensaje ?><br>
            <b>¿Deseas reenviar el comprobante?</b><br>
          </div>
        @elseif($facturacionresumen->respuestaestado=='RECHAZADA')
          <div class="mensaje-info">
            El Comprobante fue rechazado:<br>
            <?php echo $facturacionresumen->respuestacodigo ?> - <?php echo $facturacionresumen->respuestamensaje ?><br>
            <b>¿Deseas reenviar el comprobante?</b><br>
          </div>
        @elseif($facturacionresumen->respuestaestado=='EXCEPCION')
          <div class="mensaje-info">
        ...
        @endif
          <form class="js-validation-signin px-30" 
                action="javascript:;" 
                onsubmit="callback({
                  route: 'backoffice/tienda/sistema/{{ $tienda->id }}/facturacionresumendiario/{{$facturacionresumen->id}}',
                  method: 'PUT',
                  data:{
        ...
        <div class="main-register fl-wrap">
            ...
            <div class="mx-modal-cuerpo" id="contenido-enviarcorreo">
              ...
              <form class="js-validation-signin px-30" 
                  action="javascript:;" 
                  onsubmit="callback({
                    route: 'backoffice/tienda/sistema/{{ $tienda->id }}/facturacionresumendiario/0',
                    method: 'PUT',
                    carga: '#mx-carga-enviarcorreo',
                    data:{
                        view: 'enviarcorreo',
                        idfacturacionresumen: {{$facturacionresumen->id}}
                    }
                },
                function(resultado){
