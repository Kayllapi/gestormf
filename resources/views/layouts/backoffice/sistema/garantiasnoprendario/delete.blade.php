<form action="javascript:;" 
      onsubmit="callback({
          route: '{{ url('backoffice/'.$tienda->id.'/garantiasnoprendario/'.$garantias->id) }}',
          method: 'DELETE',
          data:{
              view: 'eliminar'
          }
      ...
      function(resultado){
          ...
          $('#modal-close-garantias-eliminar-nopren').click(); 
          $('#tabla-garantias').DataTable().ajax.reload();
          lista_garantias_cliente({{ $garantias->idcliente }});
          $('#btn-delete-garantia').addClass('d-none');
      },this)">
    <div class="modal-header">
        <h5 class="modal-title">Eliminar Garantia No Prendario</h5>
        <button type="button" class="btn-close" id="modal-close-garantias-eliminar-nopren" data-bs-dismiss="modal" aria-label="Close"></button>
    </div>
    <div class="modal-body">
        <div class="alert alert-danger">
          <i class="fa-solid fa-triangle-exclamation"></i> ¿Esta seguro de eliminar la garantia no prendario?<br>
