    ...
    <table class="table" id="table-cobranzapendiente">
        <thead style="background: #31353d; color: #fff;">
            <tr>
                ...
                <td style="padding: 8px;text-align: center;">Ganancia</td>
                <td style="padding: 8px;text-align: center;">Total</td>
                <td style="padding: 8px;text-align: center;">Retirar</td>
            </tr>
        </thead>
        <tbody>
        @foreach($cronograma['cuotas_pendientes'] as $value)
        <tr style="{{$value['tabla_colortr']}};">
            <td style="padding: 8px;text-align: right;width: 10px;">{{$value['tabla_numero']}}</td>
            <td style="padding: 8px;text-align: center;">{{$value['tabla_fechaahorro']}}</td>
            <td style="padding: 8px;text-align: right;">{{$value['tabla_cuota']}}</td>
            <td style="padding: 8px;text-align: right;background-color: orange;color: white;">{{$value['tabla_interesganado']}}</td>
            <td style="padding: 8px;text-align: right;">{{$value['tabla_total']}}</td>
            <td style="padding: 8px;text-align: right;">{{$value['tabla_retirar']}}</td>
        </tr>
        @endforeach
        </tbody>
        <tfoot style="background: #31353d; color: #fff;">
          <tr>
              ...
              <td style="padding: 8px;text-align: right;">{{$cronograma['total_pendiente_interesganado']}}</td>
              <td style="padding: 8px;text-align: right;">{{$cronograma['total_pendiente_total']}}</td>
              <td style="padding: 8px;text-align: right;">{{$cronograma['total_activa_retirar']}}</td>
          </tr>
        </tfoot>
    </table>
<script>
  $('#totalganancia').val('{{$cronograma['total_activa_retirar']}}');
  $('#totalgananciaredondeado').val('{{$cronograma['total_activa_retirar_redondeado']}}');
</script>
    ...
    <table class="table" id="table-cobranzapendiente">
        <thead style="background: #31353d; color: #fff;">
            <tr>
                ...
                <td style="padding: 8px;text-align: right;">Recaudación</td>
...
<script>
 ...
 /* var montorecibido = 0;
  ...
  // Resumen Credito
  $('#resumen-desembolso-monto').val('{{$cronograma['ahorrosolicitud']->monto}}');
  $('#resumen-desembolso-interes').val('{{$cronograma['ahorrosolicitud']->total_interesganado}}');
  $('#resumen-desembolso-frecuencia').val('{{$cronograma['ahorrosolicitud']->frecuencianombre}}');
  $('#resumen-desembolso-fechainicio').val('{{date_format(date_create($cronograma['ahorrosolicitud']->fechainicio),"d/m/Y")}}');
  $('#resumen-desembolso-ultimafecha').val('{{date_format(date_create($cronograma['ahorrosolicitud']->ultimafecha),"d/m/Y")}}');
  $('#resumen-desembolso-cuotafija').val('{{$cronograma['ahorrosolicitud']->monto}}');
