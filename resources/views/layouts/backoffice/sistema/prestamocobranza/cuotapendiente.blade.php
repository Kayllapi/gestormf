...
<table class="table" id="table-cobranzapendiente">
    <thead style="background: #31353d; color: #fff;">
        <tr>
            ...
            <td style="padding: 8px;text-align: center;">Pago</td>
            @if($cronograma['total_pendiente_abono']>0)
            <td style="padding: 8px;text-align: center;">Abono</td>
            @endif
        </tr>
    </thead>
    <tbody>
...
@foreach($cronograma['cuotas_pendientes'] as $value)
    <tr style="{{$value['tabla_colortr']}};" {{$value['tabla_class']}} idcreditodetalle="{{$value['idprestamo_creditodetalle']}}">
...
<script>
  $('#cuotas_totalabono').val('0.00');
  $('#cont-abono').css('display', 'none');
  ...
  @if($request->idtipopago==1)
      ...
      @if(isset($request->hastacuota))
        @if(($cronograma['select_mora']-$cronograma['morarestante']>0) && ($request->hastacuota<$cronograma['creditosolicitud']->numerocuota))
  ...
  $('#cuotas_total_mora').val('{{number_format(($cronograma['select_mora']-$cronograma['morarestante'])<0?0:($cronograma['select_mora']-$cronograma['morarestante']), 2, '.', '')}}');
 ...
 /* var montorecibido = 0;
  ...
  $('#resumen-desembolso-fechadesembolso').val('{{date_format(date_create($cronograma['creditosolicitud']->fechadesembolsado),"d/m/Y")}}');
