...
<div class="list-single-main-wrapper fl-wrap">
  <div class="breadcrumbs gradient-bg fl-wrap">
    <span>Solicitudes de Crédito Grupal</span>
    <a class="btn btn-warning" href="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/prestamosolicitudgrupal/create') }}"><i class="fa fa-angle-right"></i> Registrar</a>
  </div>
</div>
<div class="table-responsive">
          <table class="table" id="tabla-contenido">
              ...
              @include('app.tablesearch',[
                  ...
                  'search_url'=> url('backoffice/tienda/sistema/'.$tienda->id.'/prestamosolicitudgrupal')
              ])
              <tbody>
                @foreach($prestamocreditos as $value)
                  <tr>
                    <td>{{ str_pad($value->codigo, 8, "0", STR_PAD_LEFT) }}</td>
                    <td>{{$value->idprestamo_tipocredito==5?$value->tipocredito:$value->tipocreditonombre}}</td>
                    <td>{{$value->nombre}}</td>
                    ...
                    <td>
                      <div class="header-user-menu menu-option" id="menu-opcion">
                          ...
                          <ul>
                          @if($value->idestadocredito==1)
                              <li><a href="{{ url('backoffice/tienda/sistema') }}/{{$tienda->id}}/prestamosolicitudgrupal/{{$value->id}}/edit?view=editar"><i class="fa fa-edit"></i> Editar</a></li>
                              <li><a href="{{ url('backoffice/tienda/sistema') }}/{{$tienda->id}}/prestamosolicitudgrupal/{{$value->id}}/edit?view=preaprobar"><i class="fa fa-check"></i> Pre Aprobar</a></li>
                              <li><a href="{{ url('backoffice/tienda/sistema') }}/{{$tienda->id}}/prestamosolicitudgrupal/{{$value->id}}/edit?view=eliminar"><i class="fa fa-trash"></i> Eliminar</a></li>
                          @elseif($value->idestadocredito==2)
                              <li><a href="{{ url('backoffice/tienda/sistema') }}/{{$tienda->id}}/prestamosolicitudgrupal/{{$value->id}}/edit?view=detalle"><i class="fa fa-list"></i> Detalle</a></li>
                          @elseif($value->idestadocredito==3)
                              <li><a href="{{ url('backoffice/tienda/sistema') }}/{{$tienda->id}}/prestamosolicitudgrupal/{{$value->id}}/edit?view=detalle"><i class="fa fa-list"></i> Detalle</a></li>
                          @elseif($value->idestadocredito==4)
                              <li><a href="{{ url('backoffice/tienda/sistema') }}/{{$tienda->id}}/prestamosolicitudgrupal/{{$value->id}}/edit?view=detalle"><i class="fa fa-list"></i> Detalle</a></li>
