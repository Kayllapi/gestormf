...
<script>
  ...
  function show_data(id){
    $.ajax({
      url:"{{url('backoffice/0/productoahorro/showtable')}}",
      type:'GET',
      data: {
          idcliente : id
      },
  ...
  function load_form_edit(e) {
    ...
    $(e).addClass('selected');
//     modal({route:"{{url('backoffice')}}/{{$tienda->id}}/productoahorro/"+id+"/edit?view=editar"}); 
    pagina({ route:"{{url('backoffice')}}/{{$tienda->id}}/productoahorro/"+id+"/edit?view=editar", result:'#form-create-result'});
    
  }
  load_form_create();
  function load_form_create(){
    pagina({ route:"{{url('backoffice/'.$tienda->id.'/productoahorro/create?view=registrar')}}", result:'#form-create-result'});
  }
//   load_form_create();
