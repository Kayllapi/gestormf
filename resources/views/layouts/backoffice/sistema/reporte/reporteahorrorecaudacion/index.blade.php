...
<div class="list-single-main-wrapper fl-wrap">
    <div class="breadcrumbs gradient-bg fl-wrap">
      <span>Reporte de Recaudaciones</span>
    </div>
</div>
      <div class="row">
         <div class="col-md-6">
        @if(modulo($tienda->id,Auth::user()->id,'reporterecaudacion_listarporcajero')['resultado']=='CORRECTO')
        <label>Listar por</label>
        <select id="listarpor" disabled>
            <option value="3">CAJERO</option>
        </select>
            <label>Cajero</label>
            <select id="idcajero" name="idcajero" disabled>
...
function reporte(tipo){
  ...
  load('#iframe-carga'); 
  $('#iframe-tablapdf').html('<iframe onload="iframeload();" src="{{ url('backoffice/tienda/sistema/'.$tienda->id.'/reporte/reporteahorrorecaudacion/showtablapdf') }}'+
