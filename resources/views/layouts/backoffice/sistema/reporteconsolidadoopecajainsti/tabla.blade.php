<div class="modal-header">
  <h5 class="modal-title">Reporte de Arqueo y Operaciones de Caja Institucional</h5>
...
<script>
    ...
    function verpdf(){
        ...
        $('#cont_iframe_acta_aprobacion').html(' <iframe id="iframe_acta_aprobacion" src="{{ url('/backoffice/'.$tienda->id.'/reporteconsolidadoopecaja/0/edit?view=pdf_reporte') }}&corte='+corte+'&idagencia='+idagencia+'#zoom=100" frameborder="0" width="100%" height="600px"></iframe>');
    }
    function arqueocaja(){
        let corte = $('#corte').val();
        let idagencia = $('#idagencia').val();
        let url = "{{ url('backoffice/'.$tienda->id) }}/reporteconsolidadoopecaja/0/edit?view=arqueocaja&corte="+corte+"&idagencia="+idagencia;
        modal({ route: url })
    }
</script>  
