    ...
    <div class="list-single-header absolute-header fl-wrap">
        <div class="container">
            <div class="list-single-header-item" style="margin-bottom: 20px;margin-left: 5px;margin-right: 5px;">
                ...
                <h2>{{ $tienda->nombre }} <span> - </span><a href="{{ url($url_link) }}">{{ $tienda->categorianombre }}</a> </h2>
                ...
                @else
                    <a href="javascript:;" id="modal-iniciarsesion-master" class="list-post-counter single-list-post-counter"><span>{{ $recomendaciones }}</span><i class="fa fa-heart"></i></a>
                @endif
                @endif
                ...
                <div class="row">
                    <div class="col-md-7 ">
                        <div class="list-single-header-contacts fl-wrap">
                            <ul>
                                ...
                                <li><i class="fa fa-phone"></i><a  href="https://wa.me/{{ $tienda->codigotelefonicocodigo }}{{ $tienda->numerotelefono }}?text=Hola!+Le+escribo+de+la+tienda+virtual+de+{{ $tienda->nombre }}!" target="_blank">
                                ...
                                @endif
                                @if($tienda->paginaweb!='')
                                <li><i class="fa fa-globe"></i><a  href="http://{{ $tienda->paginaweb }}" target="_blank">{{ $tienda->paginaweb }}</a></li>
                                @endif
                            </ul>
                        </div>
                    </div>
                    <div class="col-md-5">
                        <div class="fl-wrap list-single-header-column">
                            ...
                            @else
                            <div class="share-holder hid-share">
                                <a href="javascript:;" id="modal-iniciarsesion" class="btn  big-btn circle-btn dec-btn flat-btn btn-primary"><i class="fa fa-user"></i> Iniciar Sesión</a>
                            </div>
                            @endif
    ...
    <div class="row">
        <div class="col-md-7">
            <div class="cssmenu" id="cssmenu">
              <ul>
                 ...
                 <li class="<?php echo (!isset($_GET['pagina'])&&count($listurl)==2)?'active':'' ?>"><a href="{{ url($url_link) }}">Inicio</a></li>
                 <li class="<?php echo (!isset($_GET['pagina'])&&count($listurl)==3)?'active':'' ?> has-sub"><a href="javascript:;">Productos/Servicios</a>
                    <ul>
                      ...
                      ?>
                            <li>
                                <a href="{{ url($url_link.'/'.str_replace(' ','-',mb_strtolower($value->nombre))) }}">{{ ucfirst(mb_strtolower($value->nombre)) }}</span></a>
                                @if(count($subcategorias)>0)
                                <ul>
                                    @foreach($subcategorias as $subvalue)
                                    <li><a href="{{ url($url_link.'/'.str_replace(' ','-',mb_strtolower($value->nombre)).'/'.str_replace(' ','-',mb_strtolower($subvalue->nombre))) }}">{{ ucfirst(mb_strtolower($subvalue->nombre)) }}</a></li>
                                    @endforeach
                                </ul>
                                @endif
                            </li>
                 ...
                 <li class="<?php echo isset($_GET['pagina'])?($_GET['pagina']=='informacion'?'active':''):'' ?> has-sub"><a href="javascript:;">Nosotros</a>
                    <ul>
                        <li class="<?php echo isset($_GET['pagina'])?($_GET['pagina']=='informacion'?'active':''):'' ?>"><a href="{{ url($url_link) }}/pagina/informacion">Información</a></li>
                        <li class="<?php echo isset($_GET['pagina'])?($_GET['pagina']=='comprobante'?'active':''):'' ?>"><a href="{{ url($url_link) }}?pagina=comprobante">Comprobante</a></li>
                        <li class="<?php echo isset($_GET['pagina'])?($_GET['pagina']=='comentario'?'active':''):'' ?>"><a href="{{ url($url_link) }}?pagina=comentario">Comentarios</a></li>
                    </ul>
                 </li>
              </ul>
            </div>
        ...
        <div class="col-md-3">
            <div class="box-widget search-widget" style="border: 0px;margin-top: 4px;margin-bottom: 2px;">
                <form action="{{ url($url_link.'/searchtienda') }}" class="fl-wrap">
        ...
        <div class="col-md-2" style="float: left;">
            <a href="javascript:;" class="btn btn-info" id="modal-carritocompra" onclick="selectcarritocompra()" style="margin-top: 4px;margin-bottom: 4px;padding: 15px;">
                <i class="fa fa-shopping-cart"></i> Carrito de compra (<span id="cantidad-pedido-carritocompra">0</span>)
            </a>
        </div>
    </div>
</div>
...
@else
<!--login form -->
<div class="main-register-wrap modal-iniciarsesion">
    ...
    <div class="main-register-holder">
        <div class="main-register fl-wrap">
            ...
            <?php 
            $rutaimagen = getcwd().'/public/backoffice/tienda/'.$tienda->id.'/logo/'.$tienda->imagen;
            $logo = url('public/backoffice/sistema/sin_imagen_redondo.png');
            if(file_exists($rutaimagen) && $tienda->imagen!=''){
                $logo = url('public/backoffice/tienda/'.$tienda->id.'/logo/'.$tienda->imagen);
            }
            ?>
            <div style="width: 100%;
                        float: left;
            ...
            <div class="tabs-container" id="tab-iniciarsesion">
                <ul class="tabs-menu">
                    <!--li class="current"><a href="#tab-1">Iniciar sesión</a></li-->
                    <!--li><a href="#tab-2">Registrarse</a></li-->
                </ul>
                <div class="tab">
                    <div id="tab-1" class="tab-content" style="display: block;">
                        ...
                        <div class="mensaje-info" style="margin-bottom:0px;margin-top:10px;">
                          Por aquí, solo podras acceder con usuario registrado en esta tienda.
                        </div>
                    </div>
                    <div id="tab-2" class="tab-content" style="display: none;">
                        <div class="custom-form">
                          <div id="mx-carga-usuario-2">
                            <form action="javascript:;" 
                                  onsubmit="callback({
                                      route: 'register',
                                      method: 'POST',
                                      carga: '#mx-carga-usuario-2',
                                      data:{
                                          idtienda: '{{$tienda->id}}'
                                      }
                                  },
                                ...
                                <label>Nombre * </label>
                                <input id="sistema_nombre" type="text">
                                <label>Apellidos *</label>
                                <input id="sistema_apellidos" type="text">
                                <label>Correo Electrónico (Usuario) *</label>
                                <input id="sistema_email" type="text">
                                <label>Telefono *</label>
                                <input id="sistema_numerotelefono" type="text">
                                <label >Contraseña *</label>
                                <input id="sistema_regis_password" type="password"  >
                                <label >Repetir Contraseña *</label>
                                <input id="sistema_regis_password_confirmation" type="password"  >
...
<!--  modal ubicacion --> 
<!--  modal enviar carrito de compra --> 
<div class="main-register-wrap modal-carritocompra">
    <div class="main-overlay"></div>
    <div class="main-register-holder">
        <div class="main-register fl-wrap">
            <div class="close-reg"><i class="fa fa-times"></i></div>
            <h3>Carrito de Compra</h3>
            <div id="contenido-carritocompra"></div>
        </div>
    </div>
</div>
<!--  fin modal enviar carrito de compra -->
<!--  modal tienda producto --> 
<div class="main-register-wrap modal-tiendaproducto">
    <div class="main-overlay"></div>
    <div class="main-register-holder">
        <div class="main-register fl-wrap">
            <div class="close-reg"><i class="fa fa-times"></i></div>
...
modal({click:'#modal-iniciarsesion'});
...
function selectcarritocompra(){
    ...
    $.ajax({
        url:"{{ url($tienda->link.'/'.(isset($s_categoria->id)?$s_categoria->id:'0').'/1') }}",
        type:'GET',
        data: {
            view : 'selectcarritocompra'
        },
