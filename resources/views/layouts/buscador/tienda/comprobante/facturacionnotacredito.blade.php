...
?>
  @if(isset($facturacionrespuesta))
    @if($facturacionrespuesta->estado=='ACEPTADA')
        ...
        <iframe id="content-ticketpdf" style="display: block" src="{{ url($tienda->link.'/pagina/comprobante/'.$facturacionnotacredito->id.'/edit?view=fnc_ticketpdf') }}#zoom=130" frameborder="0" width="100%" height="600px"></iframe>
        <iframe id="content-a4pdf" style="display: none" src="{{ url($tienda->link.'/pagina/comprobante/'.$facturacionnotacredito->id.'/edit?view=fnc_a4pdf') }}#zoom=130" frameborder="0" width="100%" height="600px"></iframe>
    @else
